<script lang="ts">
	...
	let currentGesture: string = randomGestures[Math.floor(Math.random() * randomGestures.length)];
	let currentFrame: number = 1;

	function changeGesture() {
		let newGestures = gestures.filter((item) => item !== currentGesture);

		currentGesture = newGestures[Math.floor(Math.random() * newGestures.length)];
	}
	...
	setInterval(nextImage, 150);
</script>

<main class="w-screen">
	<img
		class="absolute top-0 left-0 w-full px-8 mt-8 box-border"
		src={`/name/${currentFrame}.svg`}
		alt="Josh Pollard"
	/>
	<button class="mx-auto mt-24" on:click={changeGesture}>
		{#each gestures as gesture}
			{#each [1, 2, 3] as frame}
				<img
					class={`w-screen mx-auto opacity-70 ${
						gesture != currentGesture || frame != currentFrame ? 'hidden' : ''
					}`}
					src={`/hand-images/${gesture}/${frame}.svg`}
					alt={`${gesture} hand ${frame}`}
				/>
			{/each}
		{/each}
		<img
	...
	<section class="relative -top-[3vw] flex flex-col items-center gap-[4vw] pt-10">
		<img
			class="absolute -z-10 top-0 left-1/2 translate-x-[-50.5%] right-1 mx-auto w-screen rotate-180 opacity-70"
			src={`/wrist/${currentFrame}.svg`}
			alt="qr code"
		/>

		<a href="https://joshlucpoll.com" target="_blank" rel="noopener noreferrer">
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512" class="fill-white w-[7vw]"
		...
		<a href="https://www.instagram.com/joshlucpoll/" target="_blank" rel="noopener noreferrer">
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" class="fill-white w-[7vw]"
				><!--! Font Awesome Pro 6.3.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path
		...
		<a href="https://snapchat.com/add/joshlucpoll" target="_blank" rel="noopener noreferrer">
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="fill-white w-[7vw]"
				><!--! Font Awesome Pro 6.3.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path
		...
		<a href="https://linkedin.com/in/joshlucpoll" target="_blank" rel="noopener noreferrer">
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" class="fill-white w-[7vw]"
				><!--! Font Awesome Pro 6.3.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path
